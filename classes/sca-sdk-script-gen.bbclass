...
SCA_SDK_ALWAYS_INJECT_FUNCTIONS_PYTHON = "exec_wrap_combine.*"
...
def sdkgen_find_functions_python(dc, _input, probe=False):
    if not hasattr(sdkgen_find_functions_python, "seen"):
...
def sdkgen_reverse_expand_python(dc, _function, _overrides):
    ...
    for _ in range(1, 3):
        ...
        for m in re.finditer(r'(self\.__)*d\.getVar\((?P<var>[A-Z_0-9"]+)(,\s*False\s*)*\)', _funccode, re.MULTILINE):
...
FILES:${PN}:class-nativesdk:append = " \
    ${bindir}/sca.run.* \
    ${bindir}/sca.* \
"
